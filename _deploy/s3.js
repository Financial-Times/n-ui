...
Promise.all(versions.map(version => {
	return shellpromise(`nht deploy-static \`find . -path "./dist/*"\` --destination n-ui/test-v${version}/ --strip 1 --bucket ft-next-n-ui-prod --cache-control 'must-revalidate, max-age=3600' --monitor`)
