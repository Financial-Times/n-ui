...
module.exports = ({ appName, isProduction, directory, useLocalAppShell }) => {
	const assetHashes = loadAssetHashesJson(
		`${directory}/public/asset-hashes.json`
	);
	const nUiAssetHashes = loadAssetHashesJson(
		`${directory}/public/n-ui/asset-hashes.json`
	);
	const nUiReleaseName = nUiManager.getReleaseName(directory);
	const nUiUnhashedAssetsRoot = useLocalAppShell
		? `/__dev/assets/${appName}/n-ui/`
		: `//www.ft.com/__assets/n-ui/cached/${nUiReleaseName}/`;
	...
	const getAssetUrl = assetConfig => {
		...
		if (isNUi) {
			...
			return useLocalAppShell || !hash
				? fallback
				: `//www.ft.com/__assets/hashed/n-ui/${hash}`;
		} else {
			const fallback = `/__dev/assets/${appName}/${file}`;
			const hash = assetHashes[file];
			return !isProduction || !hash
				? fallback
				: `//www.ft.com/__assets/hashed/${appName}/${hash}`;
		}
	};
