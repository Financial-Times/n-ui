...
const calculateSize = async (content) => {
...
const concatenateStyles = (stylesheetNames) => {
	...
	if (!concatenatedStylesCache[hash]) {
		concatenatedStylesCache[hash] = stylesheetNames.reduce((str, name) => {
			...
			return `${str}${stylesheets[name].replace(
				/\/\*# sourceMappingURL=.*\*\//,
				''
			)}`;
		}, '');
		concatenatedStylesSizeCache[hash] = calculateSize(
			concatenatedStylesCache[hash]
		);
	}
	...
	if (
		stylesheetNames.length > 1 ||
		stylesheetNames[0] !== 'n-ui/head-n-ui-core'
	) {
...
const init = directory => {
	...
	stylesheets = stylesheetList
		...
		.map(name => ({
			name,
			contents: fs.readFileSync(`${directory}/public/${name}`, 'utf-8')
		}))
	...
	if (process.NODE_ENV !== 'production') {
		const paths = Object.keys(stylesheets).map(
			css => `${directory}/public/${css}.css`
		);
		chokidar
			.watch(paths)
			...
			.on('unlink', path => {
				...
				logger.warn(
					'Please note you will need to restart app if you add new head CSS files'
				);
			});
	}
};
