...
test-server-plain:
	mocha --file server/test/setup.js server/test/*.test.js server/test/**/*.test.js --exit

test-server-coverage: ## test-server-coverage: Run the unit tests with code coverage enabled.
	istanbul cover node_modules/.bin/_mocha --file server/test/setup.js --report=$(if $(CIRCLECI),lcovonly,lcov) server/test/*.test.js server/test/**/*.test.js --exit
