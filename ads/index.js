			...
			.then(() => {
				if (flags && flags.get('ads')) {
					...
					if(flags && flags.get('stickyHeaderAd')) {
						let stickyAd = new Sticky(document.querySelector('.above-header-advert'), document.querySelector('.header-ad-placeholder__top'), document.querySelector('.o-header__nav'));
						stickyAd.init();
					}
